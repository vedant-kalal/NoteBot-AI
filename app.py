# 📁 File: app.py  (Updated Streamlit WebApp Entry Point without Quiz + Audio Summary)
...
st.markdown("""
    ...
    <p style='font-size: 22px;'>Upload notes (PDF/PNG/JPG), get summaries, chat with NoteBot, and download as PDF or audio!</p>
...
if new_upload is None and st.session_state.last_uploaded_filename is not None:
    for key in ['ocr_text', 'corrected_text', 'notebot', 'summary', 'history', 'summarize_expanded', 'last_uploaded_filename']:
...
if st.session_state.corrected_text:
    # --- Summarizer Panel ---
    with st.expander("📄 Summarize Notes", expanded=True):
        if st.button("Summarize Now", disabled=st.session_state.loading):
            disable_all_buttons()
        ...
        if st.session_state.summary:
            ...
            st.download_button("🗕️ Download Summary PDF", data=pdf_file, file_name="summary.pdf", mime="application/pdf")

            audio_file = text_to_speech_audio(cleaned_summary)
    ...
    # --- Chatbot Panel ---
    with st.expander("💬 Chat with NoteBot", expanded=True):
        ...
        if st.session_state.history:
            ...
            st.download_button("🗕️ Download Chat PDF", data=pdf_file, file_name="chat.pdf", mime="application/pdf")
